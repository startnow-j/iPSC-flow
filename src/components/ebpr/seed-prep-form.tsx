...
export function SeedPrepForm({ task, batch, onSuccess }: SeedPrepFormProps) {
  ...
  const [notes, setNotes] = useState<string>(task.formData?.notes ?? task.notes ?? '')
    ...
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">种子批号</Label>
            <p className="text-sm font-medium">{batch.seedBatchNo ?? '-'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">种子代次</Label>
        ...
        <div className="space-y-2">
          <Label htmlFor="seed-notes">备注</Label>
          <Textarea
            id="seed-notes"
